...
def DB_intialize():
    ...
    if databases[1][0] != "mydb":
        ...
        cursor = db.cursor()
        cursor.execute(
            "CREATE TABLE users (ID INT AUTO_INCREMENT PRIMARY KEY , email VARCHAR(255) UNIQUE NOT NULL , user_name VARCHAR(255) NOT NULL , password VARCHAR(32) NOT NULL , ConCode VARCHAR(6), Valid BOOLEAN NOT NULL , alarms TEXT , answers VARCHAR(30), token TEXT  )")
        cursor.execute(
